...
class ModelDownloadWorker(
    ...
    private suspend fun downloadFileWithResume(urlStr: String, destFile: File, token: String?) {
        ...
        while (redirectCount < maxRedirects) {
            ...
            connection.setRequestProperty("User-Agent", "Mozilla/5.0 (Linux; Android 14) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Mobile Safari/537.36")
            connection.setRequestProperty("Accept", "*/*")
            
            // Authorization if provided, but ONLY to Hugging Face domains to prevent S3 HTTP 400 errors
            val isHfHost = currentUrl.host.endsWith("huggingface.co") || currentUrl.host.endsWith("hf.co")
            if (!token.isNullOrBlank() && isHfHost) {
            ...
            if (responseCode in 300..399) {
                ...
                currentUrl = if (location.startsWith("http")) URL(location) else URL(currentUrl, location)
